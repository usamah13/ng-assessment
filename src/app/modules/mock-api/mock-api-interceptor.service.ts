...
export class MockApiInterceptorService implements HttpInterceptor {
  constructor(
    @Inject(APP_MOCK_API_DEFAULT_DELAY) private _defaultDelay: number,
    private _appMockApiService: AppMockApiService
  ) {}

  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    // Try to get the request handler
    const { handler, urlParams } = this._appMockApiService.findHandler(
